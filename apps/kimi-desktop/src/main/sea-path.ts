...
export function resolveSeaPath(): string {
  ...
  if (app.isPackaged) {
    // In a quick-sharun AppImage the bundled SEA is also deployed as a sharun
    // wrapper at `<AppDir>/bin/kimi`, i.e. next to the app's `resources` dir.
    // Prefer it: the raw `<resources>/bin/<target>/kimi` would use the host's
    // dynamic linker instead of the AppImage's bundled libc.
    const wrapped = join(process.resourcesPath, '..', exe);
